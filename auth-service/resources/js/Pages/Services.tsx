...
export default function Services() {
  ...
  return (
    <GuestLayout title={t('services.pageTitle', 'Services | Tekrem')}>
      {/* Hero Section */}
      <section className="bg-gradient-to-b from-background to-muted py-16 md:py-24">
        <div className="container mx-auto">
          <div className="max-w-3xl mx-auto text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-6">{t('services.title', 'Our Services')}</h1>
            <p className="text-xl text-muted-foreground">
              {t('services.subtitle', 'Comprehensive technology solutions tailored to your business needs. From software development to networking, we\'ve got you covered.')}
            </p>
          </div>
        </div>
      </section>
          ...
          <Tabs defaultValue="software" className="w-full">
            <div className="flex justify-center mb-8">
              <TabsList className="grid grid-cols-2 md:grid-cols-5 w-full md:w-auto">
                <TabsTrigger value="software" className="flex items-center gap-2">
                  ...
                  <span className="hidden md:inline">{t('services.tabs.software', 'Software')}</span>
                </TabsTrigger>
                <TabsTrigger value="ai-cloud" className="flex items-center gap-2">
                  <Cloud className="h-4 w-4" />
                  <span className="hidden md:inline">{t('services.tabs.aiCloud', 'AI & Cloud')}</span>
                </TabsTrigger>
                <TabsTrigger value="networking" className="flex items-center gap-2">
                  <Network className="h-4 w-4" />
                  <span className="hidden md:inline">{t('services.tabs.networking', 'Networking')}</span>
                </TabsTrigger>
                <TabsTrigger value="consultancy" className="flex items-center gap-2">
                  <Lightbulb className="h-4 w-4" />
                  <span className="hidden md:inline">{t('services.tabs.consultancy', 'Consultancy')}</span>
                </TabsTrigger>
                <TabsTrigger value="graphics" className="flex items-center gap-2">
                  <PenTool className="h-4 w-4" />
                  <span className="hidden md:inline">{t('services.tabs.graphics', 'Graphics')}</span>
                </TabsTrigger>
              </TabsList>
            </div>
            ...
            <TabsContent value="software" className="space-y-8">
              <div className="text-center mb-8">
                <h2 className="text-3xl font-bold mb-4">{t('services.software.title', 'Software Development')}</h2>
                <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
                  {t('services.software.description', 'Custom software solutions designed to streamline your business processes, enhance productivity, and drive growth.')}
                </p>
              </div>
            ...
            <TabsContent value="ai-cloud" className="space-y-8">
              <div className="text-center mb-8">
                <h2 className="text-3xl font-bold mb-4">{t('services.aiCloud.title', 'AI & Cloud Solutions')}</h2>
                <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
                  {t('services.aiCloud.description', 'Harness the power of artificial intelligence and cloud computing to transform your business operations and gain competitive advantage.')}
                </p>
              </div>
            ...
            <TabsContent value="networking" className="space-y-8">
              <div className="text-center mb-8">
                <h2 className="text-3xl font-bold mb-4">{t('services.networking.title', 'Networking Services')}</h2>
                <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
                  {t('services.networking.description', 'Robust networking solutions to ensure secure, reliable, and high-performance connectivity for your business.')}
                </p>
              </div>
            ...
            <TabsContent value="consultancy" className="space-y-8">
              <div className="text-center mb-8">
                <h2 className="text-3xl font-bold mb-4">{t('services.consultancy.title', 'Consultancy Services')}</h2>
                <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
                  {t('services.consultancy.description', 'Expert guidance and support to help you navigate the complex technology landscape and make informed decisions.')}
                </p>
              </div>
            ...
            <TabsContent value="graphics" className="space-y-8">
              <div className="text-center mb-8">
                <h2 className="text-3xl font-bold mb-4">{t('services.graphics.title', 'Graphics Design')}</h2>
                <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
                  {t('services.graphics.description', 'Creative design solutions to enhance your brand identity and visual communication.')}
                </p>
              </div>
      ...
      <section className="py-16 bg-muted">
        <div className="container mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
              {t('services.process.description', 'We follow a structured approach to ensure the successful delivery of every project.')}
            </p>
          </div>
      ...
      <section className="py-16">
        <div className="container mx-auto">
          <div className="bg-primary text-primary-foreground rounded-lg p-8 md:p-12">
            <div className="max-w-3xl mx-auto text-center">
              <h2 className="text-3xl font-bold mb-4">{t('services.cta.title', 'Ready to Transform Your Business?')}</h2>
              <p className="text-xl mb-8">
                {t('services.cta.description', 'Contact us today to discuss how our services can help you achieve your business goals.')}
              </p>
              <Button size="lg" variant="secondary" asChild>
                <Link href={route('contact')}>
                  {t('services.cta.button', 'Get in Touch')} <ArrowRight className="ml-2 h-5 w-5" />
                </Link>
              </Button>
            </div>
          </div>
