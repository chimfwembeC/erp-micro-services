...
export const handlePermissionError = (
  ...
  if (status) {
    ...
    if (showToast) {
      toast.error(status === 401 ? 'Authentication Required' : 'Error', {
        duration: toastDuration,
        description: message
      });
    }
    ...
    if (status === 401 || status === 409) {
      if (isSsoError && ssoLoginUrl) {
        ...
        const loginUrl = `${ssoLoginUrl}?redirect=${encodeURIComponent(window.location.origin + '/auth/callback')}`;
...
export const setupPermissionErrorInterceptor = (options: PermissionErrorOptions = {}) => {
  ...
  const isSsoEnvironment = hostname.includes('tekrem.local') ||
                          hostname === 'localhost' ||
                          hostname === '127.0.0.1';

  // Get the SSO login URL based on the environment
  let ssoLoginUrl = '/auth/login';
  
  if (isSsoEnvironment) {
    // We're in the CRM service
    if (hostname.includes('crm') || window.location.port === '8001') {
      ssoLoginUrl = `${import.meta.env.VITE_AUTH_SERVICE_URL || 'http://auth.tekrem.local'}/auth/login`;
    }
  }
