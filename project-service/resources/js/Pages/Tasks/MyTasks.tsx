...
interface MyTasksProps extends PageProps {
  tasks: {
    data: Task[];
    links: any[];
    current_page: number;
...
export default function MyTasks({ auth, tasks }: MyTasksProps) {
  ...
  const handleStartTimer = (taskId: number) => {
    router.post(route('tasks.start-timer', taskId), {}, {
      onSuccess: () => {
  ...
  const handleStopTimer = (taskId: number) => {
    router.post(route('tasks.stop-timer', taskId), {
      description: 'Stopped from My Tasks page'
    }, {
      onSuccess: () => {
      ...
      <div className="py-12">
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div className="mb-6">
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbPage>{t('tasks.my_tasks')}</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </div>
          ...
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold">{t('tasks.my_tasks')}</h1>
            <div className="flex space-x-2">
              ...
              <Button asChild variant="outline">
                <Link href={route('time-logs.my-logs')}>
                  {t('time_logs.my_logs')}
                </Link>
              </Button>
            </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle>{t('tasks.my_tasks')}</CardTitle>
              <CardDescription>{t('tasks.my_tasks_description')}</CardDescription>
            </CardHeader>
            <CardContent>
              {tasks.data.length > 0 ? (
            ...
            {tasks.last_page > 1 && (
              <CardFooter>
                <Pagination
                  ...
                  onPageChange={(page) => router.visit(route('tasks.my-tasks', { page }))}
                />
              </CardFooter>
            )}
          </Card>
